...
public class AreaFactory implements Context {
    ...
    public AreaFactory(Jmezombies game) {
    ...
    public void buildTerrainEnvironment() {
        Spatial worldMap = assetManager.loadModel("Scenes/Map.gltf");


        worldNode = new Node("worldNode");
        worldNode.setShadowMode(RenderQueue.ShadowMode.CastAndReceive);

        worldMap.depthFirstTraversal(spatial -> {
...
//        fpp.addFilter(bloom);
    ...
    private NavMeshDataCreateParams getNavMeshCreateParams(JmeInputGeomProvider provider, RecastBuilderResult rcResult) {
        ...
        params.walkableHeight = AreaFactory.agentHeight;
        params.walkableRadius = AreaFactory.agentRadius;
        params.walkableClimb = AreaFactory.agentMaxClimb;
        params.bmin = polyMesh.bmin;
        params.bmax = polyMesh.bmax;
        params.cs = AreaFactory.cellSize;
        params.ch = AreaFactory.cellHeight;
